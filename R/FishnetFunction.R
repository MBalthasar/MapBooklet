...
#' vnm_shp <- raster::shapefile(system.file(package = "MapBooklet", "extdata",
#'                                          "vietnam_borders.shp"))
#'
#' # Apply diff_factor
...
FishnetFunction <- function(my_poly, extent_only, diff_factor, length_factor){
  ...
  if (substr(current_proj, 0, 9) == "+proj=utm"){
    ...
    my_extent <- methods::as(raster::extent(my_poly), 'SpatialPolygons')
  ...
  if (missing(length_factor)){
    ...
    for (i in 1:diff_factor){
      ...
      for (j in 1:diff_factor){
        ...
        sp <- methods::as(ext, 'SpatialPolygons')
