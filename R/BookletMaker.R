...
#'                     See \code{\link[MapBooklet]{FishnetFunction}}.
#'
...
BookletMaker <- function(user_ggplot, user_fishnet, fishnet_col, user_width,
  ...
  if (missing(add_scale)){
    ...
    user_proj <- raster::crs(user_fishnet)
  ...
  cover_plot <- user_ggplot +
    ggplot2::geom_polygon(data = fishnet_df, # Define data for polygon
                 ggplot2::aes(x = long, y = lat, group=group), # Define aesthetics for polygon
  ...
  for (i in 1:nrow(aoi_centroids_df)){
    cover_plot <- cover_plot + ggplot2::annotate(geom="text", # Define text as annotation type
  ...
  for (i in 1:nrow(user_fishnet)){
    ...
    suppressMessages(
      current_tile_df <- ggplot2::fortify(current_tile)
    )
    ...
    suppressMessages(
      current_plot <- user_ggplot +
        ggplot2::ggtitle(paste0("Tile ", current_id)) + # Change title
        ggplot2::theme(legend.position="none") + # Remove legend
        ggplot2::coord_fixed(xlim = c(current_xlim[1], current_xlim[2]), # Change x limits
                    ylim = c(current_ylim[1], current_ylim[2])) # Change y limits
    )
    # Add tile outlines
    current_plot <- current_plot +
      ggplot2::geom_polygon(data = current_tile_df, # Define data for polygon
                   ggplot2::aes(x = long, y = lat, group=group), # Define aesthetics for polygon
                   colour = fishnet_col, fill = NA) # Choose line colour and fill with NA
